...
### 9.2 多 Agent 运行策略（长任务上下文保护）

> 写/改代码是**长任务**，会持续耗尽主 agent 的上下文窗口。必须用子 agent 分担实现，主 agent 只做编排与验收，避免上下文耗尽导致中途失忆、返工。

**按任务类型区分触发：**

| 任务类型 | 判定 | 多 agent |
|---|---|---|
| 修改/编写代码（新增功能、修 Bug、重构、UI 开发） | 长任务，产生大量代码与上下文 | ✅ 需要（执行方式见下方分支策略） |
| 分析/阅读/研究代码（解释架构、读源码、回答问题、调研） | 短任务，只读不改 | ❌ 不需要，单 agent 直接做 |

**按分支区分策略：**
- **master（主 checkout）**：写代码时**按实际情况灵活判断**——改动跨多模块、文件多、周期长、上下文压力大 → 开启多 agent；单一文件、改动小、逻辑简单 → 可单 agent 完成。
- **域分支（domain/* worktree）**：**必须多 agent 运行**——域功能开发是长任务集中地，域 agent 上下文一旦耗尽会拖垮整条分支的产出与质量。

**实现方式**：用 `subagent-driven-development`（按计划把实现逐项派给子 agent）或 `dispatching-parallel-agents`（并行派发多个子 agent）；每个子 agent 持有自己的任务子集、独立编译测试，主 agent 只做编排、验收与汇总（配合 §9.1 完整流程与代码审查）。
